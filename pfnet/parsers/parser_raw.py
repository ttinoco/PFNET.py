...
class PyParserRAW(object):
    ...
    def parse(self, filename, num_periods=None):
        ...
        for raw_trafo in case.transformers:
            if self.keep_all_oos or raw_trafo.p1.stat > 0:
                if isinstance(raw_trafo, pd.struct.TwoWindingTransformer):
                    raw_branches.append(raw_trafo)
                elif isinstance(raw_trafo, pd.struct.ThreeWindingTransformer): # 3 Times because 3w
                    raw_branches.append(raw_trafo)      
                    raw_branches.append(raw_trafo)
                    raw_branches.append(raw_trafo)
        ...
        for index, raw_branch in enumerate(reversed(raw_branches)):
            ...
            # 2w_transformer
                ...
                trafo_2w.in_service = raw_trafo.p1.stat > 0
                ...
                if raw_branch.w1.cod == 0:
                    ...
                    trafo_2w.reg_bus = net.get_bus_from_number(raw_branch.w1.cont) # can "cont" be zero? If so, then what?
                ...
                Y = self.get_tran_series_parameters(x12, r12, raw_branch.p1.cz, tbase, case.sbase)
            ...
            # 3 Windings Transformers
               ...
               # in_service flag needs to be set dependeing on trafo_3w_count and status value
               
               # Series parameters 
               Y12 = self.get_tran_series_parameters(raw_branch.p2.x12,
                                                     raw_branch.p2.r12,
                                                     raw_branch.p1.cz,
                                                     raw_branch.p2.sbase12,
                                                     case.sbase)
               Y23 = self.get_tran_series_parameters(raw_branch.p2.x23,
                                                     raw_branch.p2.r23,
                                                     raw_branch.p1.cz,
                                                     raw_branch.p2.sbase23,
                                                     case.sbase)
               Y31 = self.get_tran_series_parameters(raw_branch.p2.x31,
                                                     raw_branch.p2.r31,
                                                     raw_branch.p1.cz,
                                                     raw_branch.p2.sbase31,
                                                     case.sbase)               
               ...
               if trafo_3w_count % 3 == 0:
                   ...
                   trafo_3w.bus_m = net.get_bus_from_number(tran2star[raw_branch.index].i)
                   trafo_3w.name = raw_branch.p1.ckt
                   
                   # Series parameters
                   Y = self.get_3w_tran_admitance(Y12, Y23, Y31, "i")
                   ...
                   if raw_branch.p1.nmetr == 1:
                       if raw_branch.p1.cm == 2: 
                            ...
                            trafo_3w.b_k = -np.sqrt((raw_branch.p1.mag2*(raw_branch.p2.sbase12/case.sbase))**2 -
                                                    raw_branch.p1.mag2**2)  
                            trafo_3w.g_m = 0
                            trafo_3w.b_m = 0 
               ...
               # j section of transformer
                   ...
                   # Series parameters
                   Y = self.get_3w_tran_admitance(Y12,Y23,Y31,"j")
                   ...
                   if raw_branch.p1.nmetr == 2:
                       if raw_branch.p1.cm == 2: 
                            trafo_3w.g_k = raw_branch.p1.mag1/(case.sbase*(10**6))
                            trafo_3w.b_k = -np.sqrt((raw_branch.p1.mag2*(raw_branch.p2.sbase23/case.sbase))**2 -
                                                    raw_branch.p1.mag2**2)               
                            trafo_3w.g_m = 0
                            trafo_3w.b_m = 0
                       else:
               ...
               # k section of transformer
                   ...
                   # Series parameters
                   Y = self.get_3w_tran_admitance(Y12,Y23,Y31,"k")
                   ...
                   if raw_branch.p1.nmetr == 3:
                       if raw_branch.p1.cm == 2:          
                            trafo_3w.g_k = raw_branch.p1.mag1/(case.sbase*(10**6))
                            trafo_3w.b_k = -np.sqrt((raw_branch.p1.mag2*(raw_branch.p2.sbase31/case.sbase))**2 -
                                                    raw_branch.p1.mag2**2)            
                            trafo_3w.g_m = 0
                            trafo_3w.b_m = 0
                       else:
               ...
               # Control Modes
               trafo_3w.set_as_fixed_tran() # for now
               #   if raw_branch.w1.cod == 0:
               #       
               #   elif raw_branch.w1.cod == 1:
        ...
        for index, raw_shunt in enumerate(reversed(raw_shunts)):
            ...
            # Switched Shunt
                ...
                self.get_shunt_b_values(raw_shunt.b1, raw_shunt.n1, b)
                self.get_shunt_b_values(raw_shunt.b2, raw_shunt.n2, b)
                self.get_shunt_b_values(raw_shunt.b3, raw_shunt.n3, b)
                self.get_shunt_b_values(raw_shunt.b4, raw_shunt.n4, b)
                self.get_shunt_b_values(raw_shunt.b5, raw_shunt.n5, b)
                self.get_shunt_b_values(raw_shunt.b6, raw_shunt.n6, b)
                self.get_shunt_b_values(raw_shunt.b7, raw_shunt.n7, b)
                self.get_shunt_b_values(raw_shunt.b8, raw_shunt.n8, b)
        ...
        for index, raw_fact in enumerate(reversed(raw_facts)):
            ...
            fact.in_service = raw_fact.mode != 0
    ...
    def write(self, net, filename):
        ...
        for bus in reversed(net.buses):
            ...
            def get_ide(bus):
                ...
                case.buses.append(pd.struct.Bus(i,
                                                name,
                                                basekv,
                                                ide,
                                                area,
        ...
        for load in reversed(net.loads):
            ...
            case.loads.append(pd.struct.Load(index,
                                             i,
                                             ID,
                                             status,
                                             area,
        ...
        for gen in net.generators:
            ...
            case.generators.append(pd.struct.Generator(index,
                                                       i,
                                                       id,
                                                       pg,
                                                       qg,
        ...
        for line in reversed(net.branches):
            ...
            if line.is_line():
               ...
               x = -line.b/den
               b = line.b_m + line.b_k
               ratea = line.ratingA*net.base_power
               rateb = line.ratingB*net.base_power
               ratec = line.ratingC*net.base_power
               gi = line.g_k
               bi = 0.  
               gj = line.g_m
               bj = 0.
               st = int(line.in_service)
               met = -1
               length = 0.
               o1 = 1
               f1 = 1.
               ...
               case.branches.append(pd.struct.Branch(index,
                                                     i,
                                                     j,
                                                     ckt,
                                                     r,
        ...
        for index, trafo_2w in enumerate(reversed(net.branches)):
            ...
            if not (trafo_2w.is_line()):
                ...
                p1 = pd.struct.TransformerParametersFirstLine(i,
                                                              j,
                                                              k,
                                                              ckt,
                                                              cw,
                ...
                ang = trafo_2w.phase
                rata = trafo_2w.ratingA*net.base_power
                ratb = trafo_2w.ratingB*net.base_power
                ratc = trafo_2w.ratingC*net.base_power
                ...
                w1 = pd.struct.TransformerWinding(index_w1,
                                                  windv,
                                                  nomv,
                                                  ang,
                                                  rata,
    ...
    def get_3w_tran_admitance(self, y12, y23, y31, part):
    ...
    def get_shunt_b_values(self, b, n, my_list):
